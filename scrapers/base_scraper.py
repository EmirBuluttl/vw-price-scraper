...
def save_records(
    ...
    for r in records:
        ...
        model_year = r.get("model_year") or (year_match.group(1) if year_match else "2026")
        ...
        if variant_row:
            ...
            conn.execute("UPDATE variants SET model_year = ? WHERE id = ? AND (model_year IS NULL OR model_year = '' OR model_year = '2025')", (model_year, variant_id))
        else:
            cursor = conn.execute("""
