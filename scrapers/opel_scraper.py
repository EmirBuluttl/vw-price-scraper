...
class OpelScraper(BaseScraper):
    ...
    def methods(self) -> list[tuple[str, Any]]:
        return [("opel_playwright_live", self._fetch_opel_playwright_live)]
    ...
    def _fetch_opel_playwright_live(self) -> list[dict]:
        ...
        try:
            ...
            for table in soup.find_all("table"):
                ...
                match = re.search(r"(Corsa|Astra|Mokka|Grandland|Combo|Vivaro|Rocks-e)", table_text, re.IGNORECASE)
                if match:
                ...
                for tr in table.find_all("tr"):
                    ...
                    variant_raw = cols[0]
                    if any(token in variant_raw.upper() for token in ["MOTOR", "ŞANZIMAN", "DONANIM"]):
                        continue
                    ...
                    for token in ["Corsa", "Astra", "Mokka", "Grandland", "Combo", "Vivaro"]:
                        if token.upper() in variant_raw.upper():
                            model_name = token
                            break
            ...
            print(f"Playwright Opel Live Scrape Error: {exc}")
